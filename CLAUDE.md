...
- `data/` — gitignored entirely; use for transient working artifacts, vessel instance data, and download caches (e.g., `data/IMO_Compendium.xlsx`, `data/imo_alignment_review.csv`)
...
## When Aligning Data Properties to IMO Standards

Use the two-step `align_imo.py` workflow. **Prerequisite:** `model/external/imo/imo_compendium_all.csv` must exist — generate it once with `uv run python scripts/extract_imo_compendium.py`.

**Step 1 — Prepare (generates review file):**
```bash
uv run python scripts/align_imo.py --prepare
```
- Parses all modules with rdflib (accurate, not regex)
- Generates up to 5 ranked IMO candidates per unlinked data property
- Writes `data/imo_alignment_review.csv` (gitignored transient)
- Re-runnable: rows already marked `confirm`/`reject`/`iso` are preserved

**Step 2 — Review then Apply:**
1. Open `data/imo_alignment_review.csv` in a spreadsheet
2. For each property, set the `action` column on the best candidate row:
...
3. Preview what will be written: `uv run python scripts/align_imo.py --apply --dry-run`
4. Apply: `uv run python scripts/align_imo.py --apply`

The `--apply` step injects `skos:notation`, `skos:altLabel`, `dcterms:description`, `dcterms:source` directly into the source `.ttl` files. It is idempotent — already-annotated properties are skipped. After applying, re-run `--prepare` to refresh the review file and check remaining gaps.
